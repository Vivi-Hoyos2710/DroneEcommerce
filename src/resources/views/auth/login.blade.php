    ...
    <div class="flex justify-center">
      <div class="w-full md:w-8/12 lg:w-6/12">
        <div class="bg-white rounded-lg shadow-md px-6 py-8 dark:border dark:bg-gray-800 dark:border-gray-700">
          <h1 class="text-2xl font-semibold text-center text-gray-800 dark:text-white mb-4">{{ __('loginView.title') }}</h1>
          <form method="POST" action="{{ route('login') }}">
            @csrf
            <div class="mb-4">
              <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('loginView.email') }}</label>
            ...
            <div class="mb-4">
              <label for="password" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('loginView.password') }}</label>
            ...
            <div class="mb-0 text-center">
              <button type="submit" class="w-full py-2 px-4 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-lg">{{ __('loginView.title') }}</button>
            </div>
          </form>
        </div>
      </div>
